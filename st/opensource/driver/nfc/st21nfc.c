...
static void st21nfc_dev_free(struct kref *ref)
{
	struct st21nfc_device *st21nfc_dev =
		container_of(ref, struct st21nfc_device, ref);
...
static ssize_t __st21nfc_dev_read(struct file *filp, char __user *buf,
...
static ssize_t __st21nfc_dev_write(struct file *filp, const char __user *buf,
				 size_t count, loff_t *offset)
{
	struct st21nfc_device *st21nfc_dev = container_of(
		filp->private_data, struct st21nfc_device, st21nfc_device);
...
static int st21nfc_dev_open(struct inode *inode, struct file *filp)
{
	...
	down_write(&st21nfc_dev->ops_rwsem);
	if (st21nfc_dev->removed) {
		ret = -ENODEV;
		goto out;
	}
	...
	if (st21nfc_dev->device_open) {
		...
		kref_get(&st21nfc_dev->ref);
	}
out:
	up_write(&st21nfc_dev->ops_rwsem);
	return ret;
}
...
static int st21nfc_release(struct inode *inode, struct file *file)
{
	...
	down_write(&st21nfc_dev->ops_rwsem);
	if (!st21nfc_dev->removed && st21nfc_dev->irq_is_attached) {
		st21nfc_disable_irq(st21nfc_dev);
	...
	up_write(&st21nfc_dev->ops_rwsem);
	kref_put(&st21nfc_dev->ref, st21nfc_dev_free);
...
static long __st21nfc_dev_ioctl(struct file *filp, unsigned int cmd,
			      unsigned long arg)
{
	struct st21nfc_device *st21nfc_dev = container_of(
		filp->private_data, struct st21nfc_device, st21nfc_device);
...
static unsigned int __st21nfc_poll(struct file *file, poll_table *wait)
{
	struct st21nfc_device *st21nfc_dev = container_of(
		file->private_data, struct st21nfc_device, st21nfc_device);
	unsigned int mask = 0;
				...
				size_t count, loff_t *offset)
{
	...
	down_read(&st21nfc_dev->ops_rwsem);
	if (st21nfc_dev->removed)
		ret = -ENODEV;
	else
		ret = __st21nfc_dev_read(filp, buf, count, offset);
	up_read(&st21nfc_dev->ops_rwsem);
				 ...
				 size_t count, loff_t *offset)
{
	...
	if (st21nfc_dev->removed)
		...
		ret = __st21nfc_dev_write(filp, buf, count, offset);
	up_read(&st21nfc_dev->ops_rwsem);
			      ...
			      unsigned long arg)
{
	...
	if (st21nfc_dev->removed)
		...
		ret = __st21nfc_dev_ioctl(filp, cmd, arg);
	up_read(&st21nfc_dev->ops_rwsem);
...
#endif
{
	...
	if (st21nfc_dev == NULL)
		return -ENOMEM;
	kref_init(&st21nfc_dev->ref);
	init_rwsem(&st21nfc_dev->ops_rwsem);
...
// QCOM and MTK54 use standard GPIO definition
	...
	if (IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		...
		st21nfc_dev->pidle_irq_is_attached = true;
	}
...
err_remove_sysfs_group:
	sysfs_remove_group(&dev->kobj, &st21nfc_attr_grp);
err_misc_register:
	mutex_destroy(&st21nfc_dev->read_mutex);
	mutex_destroy(&st21nfc_dev->irq_dir_mutex);
	if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
...
err_pidle_workqueue:
	if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		if (st21nfc_dev->pidle_irq_is_attached) {
			devm_free_irq(dev, st21nfc_dev->irq_pw_stats_idle,
				      st21nfc_dev);
			st21nfc_dev->pidle_irq_is_attached = false;
		}
		if (st21nfc_dev->st_p_wq)
...
#endif
{
	...
	down_write(&st21nfc_dev->ops_rwsem);
	st21nfc_dev->removed = true;
	wake_up_interruptible(&st21nfc_dev->read_wq);
	...
	if (st21nfc_dev->irq_is_attached) {
		devm_free_irq(&client->dev, client->irq, st21nfc_dev);
		st21nfc_dev->irq_is_attached = false;
	}
	if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_pidle)) {
		if (st21nfc_dev->pidle_irq_is_attached) {
