...
void common_mem_pools_deinit(void)
{
	int i, j;

	del_timer_sync(&power_down_timer);
	power_down_timer_pending = false;

	if (memory_pool_thread) {
		kthread_stop(memory_pool_thread);
		memory_pool_thread = NULL;
	}
	...
	for (i = 0; i < MEMORY_POOL_COUNT; i++) {
		...
		for (j = 0; j < block_counts[i]; j++) {
			...
			atomic_set(&mem_pools[i][j].allocated, 0);
		}
	...
	atomic_set(&sensor_power_count, 0);
	atomic_set(&thread_task_count, 0);
	memory_pool_state = MEMORY_POOL_STATE_DESTROYED;
}
