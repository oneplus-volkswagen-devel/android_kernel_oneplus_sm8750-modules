...
#define HISTSOH_STORAGE_SIZE 10
#define HISTSOH_MAX_MEMBERS 5
...
static int sc5891_get_ui_soh_cc(struct oplus_chg_ic_dev *ic_dev, int *ui_soh_cc)
{
	...
	if (num > HISTSOH_MAX_MEMBERS || num < 0) {
...
static int sc5891_set_historic_soh_data(struct sc5891_device *chip, const int *set_soh_data)
{
	...
	if (set_soh_data[1] < 0 || set_soh_data[1] > HISTSOH_MAX_MEMBERS) {
		chg_err("unsupported historic SOH member count: %d\n", set_soh_data[1]);
		return -EINVAL;
	}

	chg_info("write pack historic_soh_data eeprom 2 is:");
	for (i = 0; i < HISTSOH_STORAGE_SIZE; i++) {
...
{
	int i;
	u8 get_data[HISTSOH_PAGE_SIZE] = { 0 };
	int rc = 0;
	int get_len;
	...
	for (i = 0; i < len && i < HISTSOH_PAGE_SIZE; i++) {
		output_str[i] = i < get_len ? (int)get_data[i] : 0;
...
static int sc5891_ic_register(struct sc5891_device *chip)
{
	...
	rc = of_property_read_u32(chip->dev->of_node, "oplus,sc5891_fault_mitigation",
